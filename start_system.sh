...
if $STOP; then
  ...
  for pf in "$API_PID_FILE" "$SCHEDULER_PID_FILE" "$OLLAMA_PID_FILE"; do
    if [[ -f "$pf" ]]; then
      pid=$(cat "$pf")
...
if curl -sf "$OLLAMA_URL/api/tags" &>/dev/null; then
  ...
  for i in $(seq 1 30); do
    ...
    if [[ $i -eq 30 ]]; then
      error "Ollama did not start within 30s. Check $LOG_DIR/ollama.log"
    fi
  done
fi
...
echo "  ║   Scheduler + Ollama keep running            ║"
echo "  ╚══════════════════════════════════════════════╝"
echo -e "${RESET}"
...
cleanup() {
  ...
  rm -f "$API_PID_FILE"
  info "Scheduler and Ollama are still running in background."
